...
for (const col of cols){
    new Sortable(col, {
        ...
        onEnd: function (evt) {
            const vertical = evt.item
            const board = vertical.parentElement
            let order = 1
            // url = new URL(SERVER + '/api/vertical/set_order')
            url = new URL('http://localhost:8000' + '/api/vertical/set_order')
            for (const v of board.children) {
                fetch(url.toString(), {
                    method: 'POST',
                    headers: {
